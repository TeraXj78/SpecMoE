...
# Set paths
data_folder       = "......./Data/SIENA/processed"
save_folder_train = '......./Data/SIENA/processed/train.json'
save_folder_val   = '......./Data/SIENA/processed/val.json'
save_folder_test  = '/......./Data/SIENA/processed/test.json'
...
def load_subject_data(subject_folder):
    ...
    all_labels = []
    list_files = natsorted(f for f in os.listdir(subject_folder) if f.endswith('.pkl'))
    #list_files = list_files[:75]

    print(f"\n\nlist_files -- len = {len(list_files)}")
    i = 1
    for file in list_files:
        try:
            ...
            #print(eeg_data['Y'])
            all_labels.append(eeg_data['Y'])
            #print(i)
            #i+=1
        except Exception as e:
            print(f"Error loading {file}: {str(e)}")
    return subject_data, all_labels
...
def split_subject_data(subject_data, val_ratio=0.2):
    ...
    print(f"\n\nlabel_to_data.items() -- len = {len(label_to_data.items())}")
    l = 1
    train_data, val_data = [], []
...
def main():
    ...
    for subject in train_subjects:
        subject_data, train_val_labels = load_subject_data(subject)
        all_train_val_labels = all_train_val_labels + train_val_labels
        ...
        print(subject)
        print("---------------")
        print(all_train_val_labels)
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    
    #with open('......./Data/SIENA/processed/training_labels_2.pkl', "wb") as f:
    #    pickle.dump(all_train_val_labels, f)
    ...
    for subject in test_subjects:
        all_test_data.extend(load_subject_data(subject)[0])
